...
NFA<std::string, std::string> createNFA(std::istream& is){
    NFA<std::string, std::string> nfa;
    // set epsilon
    nfa.setEpsilon("?");
    ...
    while (std::getline(is, line)) {
        ...
        std::stringstream s(line);
        s >> left;
        if(!nfa.isValidState(t)){continue;}
        s >> temp_char;
        if(std::isspace(temp_char)){s >> temp_char;}
        if(temp_char == EOF){continue;}
        if(temp_char != delimiter){continue;}
        s >> symbol;
        if(!nfa.isValidAlphabet(symbol)){continue;}
        s >> temp_char;
        if(std::isspace(temp_char)){s >> temp_char;}
        if(temp_char == EOF){continue;}
        if(temp_char != arrow){continue;}
        while(s >> right){
            if(nfa.isValidState(right))nfa.addTransition(left, symbol, right);
        }
    }
    return nfa;
}
